...
function SubscriptionPage() {
  ...
  const simulate = async () => {
    ...
    await activate.mutateAsync(method);
    setProcessing(false);
  };
  ...
  const headlineByStatus: Record<string, { title: string; sub: string }> = {
    pendente: {
      ...
      sub: "Conclua a assinatura mensal para liberar o acesso ao AgroGestor.",
    },
    vencida: {
      title: "Sua assinatura venceu",
      sub: "Renove para continuar usando o sistema.",
    },
    atrasada: {
      title: "Pagamento em atraso",
      sub: "Regularize sua assinatura para liberar o acesso.",
    },
        ...
        <Card className="mb-6 rounded-2xl border-primary/20 bg-gradient-to-br from-primary/10 to-transparent">
          <CardContent className="flex items-center justify-between p-5">
            <div>
              ...
              <div className="text-3xl font-bold text-primary">{brl(PRICE)}</div>
          ...
          <CardContent>
            <Tabs value={method} onValueChange={(v) => setMethod(v as typeof method)}>
              ...
              <TabsContent value="credito" className="pt-4">
                <CardView label="Cartão de Crédito" />
              </TabsContent>
              <TabsContent value="debito" className="pt-4">
                <CardView label="Cartão de Débito" />
              </TabsContent>
            ...
            >
              {processing ? (
                ...
                <>Confirmar pagamento de {brl(PRICE)}</>
              )}
            </Button>
  ...
  return (
    <div className="space-y-3 text-center">
      ...
      <p className="text-xs text-muted-foreground">
        Aponte a câmera do app do banco para o QR Code ou copie a chave.
      </p>
    </div>
  );
}
      ...
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-primary to-primary/60 p-5 text-primary-foreground">
        ...
        <div className="mt-4 flex justify-between text-xs">
          <div>
            ...
            <div className="font-medium">{name || "NOME DO TITULAR"}</div>
          </div>
          <div>
            <div className="opacity-70">Validade</div>
            <div className="font-medium">{exp || "MM/AA"}</div>
          </div>
        </div>
      </div>
...
function BoletoView() {
  const line =
    "23793.38128 60082.512345 67890.123456 7 89540000012000";
  return (
    <div className="space-y-3">
      <div className="rounded-xl border border-border bg-card p-4">
        <div className="flex items-center gap-2">
          <Barcode className="h-6 w-6" />
          <div className="text-sm font-semibold">Boleto bancário</div>
        </div>
